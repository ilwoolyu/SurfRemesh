...
float Vertex::operator[] (const int id)
{
	return m_vertex[id];
}
...
float Normal::operator[] (const int id)
{
	return m_normal[id];
}
...
int Face::operator[] (const int id)
{
	return m_list[id];
}
...
void Mesh::setColor(const float *v)
{
	for (int i = 0; i < m_nVertex; i++) m_vertex[i]->setColor(v);
}
...
const int Mesh::nFace(void)
{
	return m_nFace;
}

const int Mesh::nVertex(void)
{
	return m_nVertex;
}
...
void Mesh::openFile(const char *filename)
{
	...
	MeshIO *mesh;
	if (!strcmp(format, "obj")) mesh = new ObjIO(filename);
...
void Mesh::colormap(const char *color, bool overlay)
{
	char buf[255];

	ifstream fin(color);
	cout << "Color Filename: " << color << endl;
	if(fin.fail())
	{
		cout << "Failure to open " << color << endl;
		return;
	}
	float max; memset(&max, 1, sizeof(float)); max *= -1;
	float min; memset(&min, 1, sizeof(float));
	float *c = new float[m_nVertex];
	for (int i = 0; i < m_nVertex && !fin.eof(); i++)
	{
	...
	for (int i = 0; i < m_nVertex; i++)
	{
		...
		/*float v[3];
		float threshold = 7 - min;
		if (m_vertex[i]->color()[0] + min > threshold)
		{
			v[0] = 0; v[1] = 0; v[2] = 1;
		}
		else
		{
			v[0] = (threshold - (m_vertex[i]->color()[1] - min)) / threshold;
			v[1] = (threshold - (m_vertex[i]->color()[1] - min)) / threshold;
			v[2] = 1;
		}*/
		/*v[2] = (m_vertex[i]->color()[0] - min) / max;
		v[1] = 0;
		v[0] = 1 - v[2];*/
		//v[0] = 1;
		if (overlay)
		{
			float mv[3] = {(v[0] + m_vertex[i]->color()[0]) / 2, (v[1] + m_vertex[i]->color()[1]) / 2, (v[2] + m_vertex[i]->color()[2]) / 2};
			m_vertex[i]->setColor(mv);
		}
		else m_vertex[i]->setColor(v);
	}
...
void Mesh::saveFile(const char *filename, const char *format, bool normal)
{
	...
	{
		ObjIO::save(filename, this, normal);
	}
	else if (!strcmp(format, "vtk"))
	{
		VtkIO::save(filename, this, normal, true);
	}
	else
	{
		cout << "Not supoorted format!" << endl;
